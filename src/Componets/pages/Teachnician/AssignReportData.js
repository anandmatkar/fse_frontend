...
import {
  Technician_DetailJobAssign,
  Technician_DeleteReport,
...
const AssignReportData = () => {
  ...
  const deleteReport = async (report, projectId) => {
    try {
      ...
      const response = await axios.get(
        `${Technician_DeleteReport}?projectId=${projectId}&reportId=${report.id}`,
        config
      );
  ...
  const shouldShowButtons = () => {
    return project.every(
      (report) => !report.is_requested_for_approval && !report.is_approved
    );
  };
    ...
    <Layout4>
            ...
            {shouldShowButtons() && (
              <>
                <NewReportModal
                  projectID={projectID}
                  machineID={machineID}
                  onNewReport={onNewReportCallback}
                />
      ...
      >
        ...
        <Modal.Footer>
          ...
          <Button
            variant="danger"
            onClick={() => deleteReport(reportToDelete, projectID)}
          >
            Confirm Delete
          </Button>
        </Modal.Footer>
