...
function EditProjectAddMachineModal({ show, onHide, project, onSubmit }) {
  ...
  const validationSchema = Yup.object().shape({
    machineFormDataList: Yup.array().of(
      Yup.object().shape({
        MachineType: Yup.string().required("Machine Type is required"),
        MachineSerial: Yup.string().required("Serial Number is required"),
        hourCount: Yup.string().required("Hour Count is required"),
        nomSpeed: Yup.string().required("Nominal Speed is required"),
        actSpeed: Yup.string().required("Actual Speed is required"),
        techIds: Yup.array().required("Technician is required"),
        machineAttach: Yup.mixed(),
      })
    ),
  });
  ...
  const handleSubmit = async (values, actions) => {
    try {
      const machineFormDataList = values.machineFormDataList;
      const machineFormData = new FormData();
      machineFormDataList.forEach((item, index) => {
        machineFormData.append(`MachineType${index}`, item.MachineType);
        machineFormData.append(`MachineSerial${index}`, item.MachineSerial);
            ...
            {({ values, handleChange, handleSubmit, setFieldValue }) => (
              <FormikForm>
                ...
                <Modal.Footer>
                  ...
                  <Button type="submit">Save Machines</Button>
                </Modal.Footer>
              </FormikForm>
            )}
          </Formik>
